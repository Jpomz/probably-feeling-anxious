...
# Fig 3A-D
fig.3a <- fig_fun(data = dat$random, mainlab = "Random") +
  annotate(geom = "text", x = -5.6, y = 0.22, 
           label = "A", size = 6)
fig.3b <- fig_fun(data = dat$scaled, mainlab = "Scaled") +
  annotate(geom = "text", x = -5.6, y = 0.075,
           label = "B", size = 6)
fig.3c <- fig_fun(data = dat$corr, mainlab = "Correlated") +
  annotate(geom = "text", x = -5.6, y = 0.17,
           label = "C", size = 6)
fig.3d <- fig_fun(data = dat$s.c, mainlab = "Scaled & correlated") +
  annotate(geom = "text", x = -5.6, y = 0.11,
           label = "D", size = 6)

tiff(filename = "figures/fig_3.tiff",
